...
var tags_matcher = matcher(/^\s*(@.+)\s*$/, 1);
...
function parse(text) {
  ...
  return ('' + text).split(/\r?\n/g).reduce(function(features, line) {
    if (tags_matcher.is(line)) {
      tags = tags.concat(tags_matcher.get().replace(/\s*$/, '').replace(/@/g, '').split(' ')).filter((elem, pos, arr) => {
        return arr.indexOf(elem) == pos;
      })
    }
    else if (feature_matcher.is(line)) {
      features.push({
